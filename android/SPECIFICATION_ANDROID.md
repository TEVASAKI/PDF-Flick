...
2. **フリック操作** — 右フリックで保存、左フリックでゴミ箱
3. **Undo/Redo** — 最大50件の操作履歴
4. **ゴミ箱管理** — 削除ファイルの退避・復元・完全削除
5. **保存先フォルダ設定** — Storage Access Framework でフォルダ選択
...
| ファイル移動失敗 | 同名ファイルが保存先に存在 | Alert でエラーメッセージ表示 |
...
### v1.2
- ゴミ箱の複数ファイル一括復元・削除
- 保存先フォルダからの `saveFolderPath` を `index.tsx` に反映

### v2.0
- PDFサムネイル表示（`react-native-pdf` 統合）
- ダークモード完全対応
- バッチ処理（複数ファイル選択・一括操作）
